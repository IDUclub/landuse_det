...
class UrbanAPIAccess:
    ...
    async def get_projects_base_scenario_id(self, project_id: int) -> int:
        """
        ...
        scenarios = await self.requests_handler.get(endpoint)
    ...
    async def get_functional_zone_sources(
        self, scenario_id: int, source: str = None, year: int = None, is_context: bool = False
    ) -> dict:
        """
        ...
        filtered_sources = self._filter_sources(response, source, year)

        if filtered_sources:
            return await self._form_source_params(filtered_sources)
        ...
        return await self._form_source_params(response)


    def _filter_sources(
        self, sources: list[dict], source: str | None, year: int | None
    ) -> list[dict]:
        filtered = sources
    ...
    async def _form_source_params(self, sources: list[dict]) -> dict:
        """
    ...
    async def get_functional_zones_scenario_id(
        ...
        source_data = await self.get_functional_zone_sources(
    ...
    async def get_all_physical_objects_geometries(
        self, scenario_id: int, is_context: bool = False
    ) -> dict:
        """
    ...
    async def get_all_physical_objects_geometries_type_id(
        self, scenario_id: int, object_type_id: int
    ) -> dict:
        return await self.requests_handler.get(
    ...
    async def get_functional_zones_scen_id_percentages(
        ...
        http_exception: If the response is empty or the specified source is not available.
        """
        source_data = await self.get_functional_zone_sources(scenario_id, source)
    ...
    async def get_functional_zone_sources_territory_id(
        self, territory_id: int, source: str = None
    ) -> dict:
        """
        Fetch available functional zone sources for a given scenario ID and determine the best source.
    ...
    async def get_functional_zones_territory_id(
        ...
        http_exception: If the response is empty or the specified source is not available.
        """
        source_data = await self.get_functional_zone_sources_territory_id(territory_id, source)
        ...
        response = await self.requests_handler.get(endpoint, params=params)
        if not response:
            raise http_exception(
    ...
    async def get_target_cities(self, territory_id: int) -> dict:
    ...
    async def get_physical_objects_from_territory(self, territory_id: int) -> dict:
        """
    ...
    async def get_physical_objects_without_geometry(
        self, territory_id: int, params: dict = None
    ) -> dict:
    ...
    async def check_urbanization_indicator_exists(self, territory_id: int) -> dict | None:
        """
        ...
        data = await self.requests_handler.get(endpoint, ignore_404=True)
        if not data:
            return None
        return data


    async def check_indicator_exists(self, territory_id: int, indicator_id: int) -> dict | None:
        """
    ...
    async def get_indicator_values(
        self, territory_id: int, indicator_id: int, params: dict = None
    ) -> dict | None:
        endpoint = (
            f"/api/v1/territory/{territory_id}/indicator_values"
            f"?indicator_ids={indicator_id}"
    ...
    async def put_indicator_value(self, indicator_data: dict) -> dict:
        """
    ...
    async def get_physical_objects_from_territory_parallel(
        ...
        based on the provided page_size and uses the get() method from self.urban_db_api
        ...
        async def fetch_page_with_sem(url: str) -> dict:
            async with semaphore:
                data = await self.requests_handler.get(url)
    ...
    async def get_territory_boundaries(self, territory_id: int) -> dict:
    ...
    async def get_service_type_id_through_indicator(self, indicator_id: int) -> int:
    ...
    async def get_service_count(self, territory_id: int, service_type_id: int) -> int:
    ...
    async def get_services_geojson(
        self, territory_id: int, service_type_id: int, params: dict = None
    ) -> dict:
    ...
    async def check_project_indicator_exist(
        self, project_id: int, indicator_id: int
    ) -> dict | None:
        """
        Attempts to retrieve an existing indicator from the database.
        ...
        Returns:
          ...
          - None: If the response status is 404 (i.e., the indicator does not exist).
        """
        scenario_id = await self.get_projects_base_scenario_id(project_id)
        endpoint = (
