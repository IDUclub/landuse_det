...
async def get_territory_renovation_potential(
    ...
    cache_file = caching_service.get_recent_cache_file(
        cache_name, {"profile": "no_profile", "source": source_key}
    )
    ...
    if services_gdf.empty:
        ...
        combined_df = pd.concat(
            [physical_objects, services_gdf], ignore_index=True, sort=False
        )
        combined_gdf = gpd.GeoDataFrame(
            combined_df, geometry="geometry", crs=physical_objects.crs
        )
    ...
    if "service_type_id" in physical_objects.columns:
        ...
        if not high_objs.empty:
            ...
            high_join = gpd.sjoin(
                zones, high_objs[["geometry"]], how="inner", predicate="intersects"
            )

            if not high_join.empty:
                high_zone_ids = high_join.index.unique()

                zones.loc[high_zone_ids, "Уровень урбанизации"] = (
    ...
    caching_service.save_with_cleanup(
        result_json, cache_name, {"profile": "no_profile", "source": source_key}
    )

    return landuse_polygons


async def compute_urbanization_indicator(
    polygons_gdf: gpd.GeoDataFrame, territory_id: int
) -> dict:
    """
    ...
    if "Процент урбанизации" in polygons_gdf.columns:
        polygons_gdf = polygons_gdf.rename(
            columns={"Процент урбанизации": "Уровень урбанизации"}
        )
...
async def get_territory(
    territory_id: int, source: str = None, force_recalculate: bool = False
) -> dict:
    """
    ...
    logger.info(
        f"Recalculating indicator for territory {territory_id} (either forced or not found)"
    )
    landuse_polygons = await get_territory_renovation_potential(
        territory_id, source=source
    )
    computed_indicator = await compute_urbanization_indicator(
