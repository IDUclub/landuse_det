...
class RenovationPotential:
    ...
    def calculate_building_percentages(self, buildings_gdf: gpd.GeoDataFrame) -> pd.Series:
        ...
        if buildings_gdf.empty:
            return pd.Series(
                {"ИЖС": 0, "Малоэтажная": 0, "Среднеэтажная": 0, "Многоэтажная": 0}
            )
        ...
        percentages = category_counts.reindex(
            ["ИЖС", "Малоэтажная", "Среднеэтажная", "Многоэтажная"], fill_value=0
        )
    ...
    def calculate_profiled_building_area(
        self, buildings_in_zone: gpd.GeoDataFrame, zone: gpd.GeoSeries, profile_types: list
    ) -> float:
        """
        ...
        """
        if (
            not hasattr(zone, "geometry")
            or not isinstance(zone.geometry, (Polygon, MultiPolygon))
            or zone.geometry.area == 0
        ):
            return 0

        zone_area = zone.geometry.area
        profiled_buildings = buildings_in_zone[
            buildings_in_zone["object_type"].isin(profile_types)
            & buildings_in_zone.geometry.notnull()
        ]
    ...
    def calculate_total_building_area(self, buildings_in_zone, zone):
        """
    ...
    async def assign_development_type(
        ...
        missing_columns = [
            col
            for col in required_columns + development_types
        ...
        development_values = landuse_polygons[development_types].apply(
            pd.to_numeric, errors="coerce"
        )
        landuse_polygons["Застройка"] = development_values.idxmax(axis=1).where(
            development_values.max(axis=1) > 0.0
        )
        ...
        conditions = [
            ...
            (landuse_polygons["Процент профильных объектов"] >= 70.0)
            & (landuse_polygons["Процент профильных объектов"] < 90.0),
            (landuse_polygons["Процент профильных объектов"] >= 90.00),
        ]

        urbanization_levels = [
            "Высоко урбанизированная территория",  # для Residential с Многоэтажной > 30%
            "Высоко урбанизированная территория",  # для Residential с Среднеэтажной > 30%
            "Высоко урбанизированная территория",  # для Special
            "Мало урбанизированная территория",  # данных нет
            "Мало урбанизированная территория",  # 0%
            "Мало урбанизированная территория",  # <10%
            "Слабо урбанизированная территория",  # <15%
            "Средне урбанизированная территория",  # <30%
            "Хорошо урбанизированная территория",  # <70%
            "Хорошо урбанизированная территория",  # [70, 90)
            "Высоко урбанизированная территория",  # >=90%
        ]

        landuse_polygons["Уровень урбанизации"] = np.select(
            conditions, urbanization_levels, default="Мало урбанизированная территория"
        )
    ...
    async def analyze_geojson_for_renovation_potential(
        ...
        conditions = [
            ...
            & (
                landuse_polygons["Уровень урбанизации"]
        ...
        renovation_area = landuse_polygons.loc[
            landuse_polygons["Потенциал"].isnull(), "Площадь"
        ].sum()

        landuse_polygons["Неудобия"] = (
            (renovation_area / total_area * 100) if total_area > 0 else 0
        )
    ...
    def _calc_building_percentages_core(self, storeys: np.ndarray) -> dict[str, float]:
        """
        ...
        return dict(zip(["ИЖС", "Малоэтажная", "Среднеэтажная", "Многоэтажная"], pct))


    def calculate_building_percentages_optimized(
        ...
        mask = (buildings_gdf["object_type"] == "Жилой дом") & buildings_gdf[
            "storeys_count"
        ].notna()
        arr = buildings_gdf.loc[mask, "storeys_count"].to_numpy(dtype=float)
        result = self._calc_building_percentages_core(arr)
    ...
    def calculate_profiled_by_criteria(
        ...
        obj_ids = [
            c["physical_object_type_id"]
            for c in criteria_list
            if c.get("physical_object_type_id") is not None
        ]
        srv_ids = [
            c["service_type_id"]
            for c in criteria_list
            if c.get("service_type_id") is not None
        ]
    ...
    async def process_zones_with_bulk_update(
        ...
        def _sync_bulk(zones_gdf, phys_gdf, mapping):
            ...
            if joined.empty:
                ...
                for c in metric_cols:
                    result[c] = 0.0
                return result.drop(columns=["zone_id", "zone_area"]).to_crs(zones_gdf.crs)

            agg_list = []
            for zone_id, group in joined.groupby("zone_id"):
                pct = self.calculate_building_percentages(group)
                criteria = mapping.get(zones.at[zone_id, "landuse_zone"], [])
                prof_pct = (
                    self.calculate_profiled_by_criteria(
                        group,
                ...
                total_pct = self.calculate_total_building_area(group, zones.loc[zone_id])
        ...
        return await asyncio.to_thread(
            _sync_bulk, landuse_polygons, physical_objects, zone_mapping
        )
    ...
    async def get_renovation_potential(
        ...
        if not oop_objects.empty:
            ...
            if not oop_join.empty:
                ...
                zones.loc[zones["functional_zone_id"].isin(oop_zone_ids), "Потенциал"] = (
                    "Не подлежащие реновации"
                )
                zones.loc[
                    zones["functional_zone_id"].isin(oop_zone_ids), "Процент урбанизации"
                ] = "Высоко урбанизированная территория"
        ...
        joined = gpd.sjoin(renovated, buffered_gdf, how="inner", predicate="intersects")
        if joined.empty:
            logger.info(
                "No intersections between buffers and polygons were found,"
            ...
            try:
                joined["intersection_area"] = joined.apply(
                    lambda row: (
                        ...
                        if not row.geometry.intersection(
                            buffered_gdf.loc[row.index_right].geometry
                        ).is_empty
                        else 0
                    ),
                ...
                raise http_exception(
                    500,
                    "Error while searching for intersections between buffers and polygons",
                    e,
                )
    ...
    async def filter_response(
        self, polygons_gdf: gpd.GeoDataFrame, filter_type: bool = False
    ) -> gpd.GeoDataFrame:
        """
        ...
        if filter_type:  # реновация
            ...
            polygons_gdf["Потенциал реновации"] = polygons_gdf[
                "Потенциал реновации"
            ].fillna("Не подлежащие реновации")
            polygons_gdf.geometry = await SpatialMethods.round_coords_geom(
                polygons_gdf.geometry, 6
            )
        else:  # урбанизация
            required_columns = [
                "Тип землепользования",
    ...
    async def calculate_zone_percentages(
        self, scenario_id: int, is_context: bool = False, source: str = None, year: int = None
    ) -> dict:
        """
        ...
        green_objects = physical_objects_dict["green_objects"]
        forests = physical_objects_dict["forests"]
        utm_crs = landuse_polygons.estimate_utm_crs()
        landuse_polygons = landuse_polygons.to_crs(utm_crs)

        landuse_polygons["landuse_zone"] = (
            landuse_polygons["landuse_zone"]
            .replace({None: "Unknown", "null": "Unknown"})
            .fillna("Unknown")
        )
        ...
        zone_percentages = (
            zone_area / (total_area_landuse + water_objects + green_objects + forests) * 100
        ).to_dict()

        water_percentage = (
            water_objects / (total_area_landuse + water_objects + green_objects + forests)
        ) * 100
        green_objects_percentage = (
            green_objects / (total_area_landuse + water_objects + green_objects + forests)
        ) * 100
        forests_percentage = (
            forests / (total_area_landuse + water_objects + green_objects + forests)
        ) * 100
        ...
        filtered_zone_percentages["Иные категории земель"] = round(
            other_categories_total, 2
        )
    ...
    async def get_projects_renovation_potential(
        ...
        discomfort_value = (
            ...
            if "Неудобия" in landuse_polygons.columns
        ...
        response = {"geojson": geojson, "discomfort": discomfort_value}
    ...
    async def get_projects_landuse_parts_scen_id_main_method(
        ...
        landuse_parts = await self.calculate_zone_percentages(
