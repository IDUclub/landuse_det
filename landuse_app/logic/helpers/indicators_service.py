...
class IndicatorsService:
    ...
    async def population_density(
        ...
        ros = [
            rec for rec in population_data if rec.get("information_source") == "РОССТАТ"
        ]
        if ros:
            ...
            logger.info(
                f"Fallback population source: '{chosen['information_source']}' on {chosen['date_value']}"
            )
    ...
    async def city_size_indicator(
        ...
        if not force_recalculate:
            ...
            if existing is not None:
                logger.info(
                    f"Returning cached indicator {indicator_id} for territory {territory_id}"
                )
                return existing
    ...
    async def oopt_parts(territory_id: int, force_recalculate: bool = False):
        ...
        if not features:
            ...
            if recreation_area == 0:
                recreation_part = 0.0
                logger.warning(
                    f"OOPT area=0 for ID:{territory_id},  recreation_part forced to 0.0%"
                )
            else:
